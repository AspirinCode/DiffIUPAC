...
def create_iupac_smiles_tokenizer(return_pretokenized, path, tokenizer_ckpt: str = './data'):

    if return_pretokenized:
        print(f'*******use pretrained iupac_tokenizer*****{return_pretokenized}*******')
        print(pt.join(tokenizer_ckpt,"real_iupac_tokenizer.pt"))
        iupac_tokenizer  = get_iupac_tokenizer(is_train=1,full_path =tokenizer_ckpt)
        smiles_tokenizer = get_smiles_tokenizer(is_train=1,checkpoint = "./data/smile_tocken")
        return iupac_tokenizer,smiles_tokenizer
    else:
        return None,None
